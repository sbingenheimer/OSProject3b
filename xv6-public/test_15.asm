...
main(int argc, char *argv[])
{
   ...
   e:	51                   	push   %ecx
   ppid = getpid();
   f:	e8 d1 04 00 00       	call   4e5 <getpid>

   lock_init(&lock);
  14:	83 ec 0c             	sub    $0xc,%esp
  17:	68 44 0d 00 00       	push   $0xd44
   ppid = getpid();
  1c:	a3 4c 0d 00 00       	mov    %eax,0xd4c
   lock_init(&lock);
  21:	e8 fa 03 00 00       	call   420 <lock_init>

   int i;
   for (i = 0; i < num_threads; i++) {
  26:	a1 40 0d 00 00       	mov    0xd40,%eax
  ...
  40:	a1 40 0d 00 00       	mov    0xd40,%eax
  ...
  99:	5b                   	pop    %ebx
  9a:	ff 35 4c 0d 00 00    	push   0xd4c
  a0:	e8 f0 03 00 00       	call   495 <kill>
  a5:	e8 bb 03 00 00       	call   465 <exit>
   for (i = 0; i < num_threads; i++) {
  ...
  b8:	a1 40 0d 00 00       	mov    0xd40,%eax
  bd:	83 c3 01             	add    $0x1,%ebx
  c0:	39 d8                	cmp    %ebx,%eax
  c2:	7e 26                	jle    ea <main+0xea>
      int join_pid = thread_join();
  c4:	e8 37 03 00 00       	call   400 <thread_join>
      assert(join_pid > 0);
  ...
  ea:	0f af 05 3c 0d 00 00 	imul   0xd3c,%eax
  f1:	3b 05 48 0d 00 00    	cmp    0xd48,%eax
 ...
 122:	e8 a9 04 00 00       	call   5d0 <printf>
   exit();
 127:	e8 39 03 00 00       	call   465 <exit>
...
worker(void *arg1, void *arg2) {
 ...
 134:	83 ec 04             	sub    $0x4,%esp
   int i, j, tmp;
   for (i = 0; i < loops; i++) {
 137:	a1 3c 0d 00 00       	mov    0xd3c,%eax
 ...
 14b:	83 c3 01             	add    $0x1,%ebx
      lock_acquire(&lock);
 14e:	68 44 0d 00 00       	push   $0xd44
 153:	e8 d8 02 00 00       	call   430 <lock_acquire>
      tmp = global;
      for(j = 0; j < 50; j++); // take some time
      global = tmp + 1;
      lock_release(&lock);
 158:	c7 04 24 44 0d 00 00 	movl   $0xd44,(%esp)
      global = tmp + 1;
 15f:	83 05 48 0d 00 00 01 	addl   $0x1,0xd48
      lock_release(&lock);
 166:	e8 e5 02 00 00       	call   450 <lock_release>
   for (i = 0; i < loops; i++) {
 16b:	83 c4 10             	add    $0x10,%esp
 16e:	39 1d 3c 0d 00 00    	cmp    %ebx,0xd3c
 ...
 2a9:	31 db                	xor    %ebx,%ebx
{
 ...
 2b5:	57                   	push   %edi
 2b6:	6a 00                	push   $0x0
 2b8:	e8 c0 01 00 00       	call   47d <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 31d:	e8 83 01 00 00       	call   4a5 <open>
  if(fd < 0)
 ...
 331:	50                   	push   %eax
 332:	e8 86 01 00 00       	call   4bd <fstat>
  close(fd);
 337:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 33a:	89 c6                	mov    %eax,%esi
  close(fd);
 33c:	e8 4c 01 00 00       	call   48d <close>
  return r;
...
int thread_create(void (*start_routine)(void *, void *), void *arg1, void *arg2){
 ...
 3db:	e8 20 04 00 00       	call   800 <malloc>
  void * stack;

  //setting up pagealignment
  if (((uint)stackspace % PGSIZE) == 0){
 3e0:	83 c4 10             	add    $0x10,%esp
 3e3:	a9 ff 0f 00 00       	test   $0xfff,%eax
 3e8:	74 05                	je     3ef <thread_create+0x1f>
    stack = stackspace;
  }else{
    stack = stackspace + (PGSIZE - ((uint)stackspace - PGSIZE));
 3ea:	b8 00 20 00 00       	mov    $0x2000,%eax
  }

  //call clone
  int toret = clone(start_routine, arg1, arg2, stack);
 3ef:	50                   	push   %eax
 3f0:	ff 75 10             	push   0x10(%ebp)
 3f3:	ff 75 0c             	push   0xc(%ebp)
 3f6:	ff 75 08             	push   0x8(%ebp)
 3f9:	e8 07 01 00 00       	call   505 <clone>
  if (toret != -1){
    return toret;
  }else {
    return -1;
...
int thread_join() {
 ...
 409:	50                   	push   %eax
 40a:	e8 fe 00 00 00       	call   50d <join>
    return toret;
  }
  else{
    return -1;
 ...
 52c:	89 d1                	mov    %edx,%ecx
{
 ...
 559:	0f b6 92 d4 09 00 00 	movzbl 0x9d4(%edx),%edx
 ...
 59e:	56                   	push   %esi
 59f:	57                   	push   %edi
 5a0:	e8 e0 fe ff ff       	call   485 <write>
  while(--i >= 0)
 ...
 5d9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 615:	57                   	push   %edi
 616:	56                   	push   %esi
 617:	e8 69 fe ff ff       	call   485 <write>
 ...
 649:	ff 24 85 7c 09 00 00 	jmp    *0x97c(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 66e:	e8 12 fe ff ff       	call   485 <write>
        putc(fd, c);
 ...
 67f:	57                   	push   %edi
 680:	56                   	push   %esi
 681:	e8 ff fd ff ff       	call   485 <write>
        putc(fd, c);
 ...
 6eb:	57                   	push   %edi
 6ec:	56                   	push   %esi
 6ed:	e8 93 fd ff ff       	call   485 <write>
        while(*s != 0){
 ...
 72c:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 72f:	e8 51 fd ff ff       	call   485 <write>
        ap++;
 ...
 757:	90                   	nop
          s = "(null)";
 758:	ba 72 09 00 00       	mov    $0x972,%edx
        while(*s != 0){
...
free(void *ap)
{
 ...
 771:	a1 50 0d 00 00       	mov    0xd50,%eax
{
 ...
 7b1:	5b                   	pop    %ebx
  freep = p;
 7b2:	89 15 50 0d 00 00    	mov    %edx,0xd50
}
 7b8:	5e                   	pop    %esi
 7b9:	5f                   	pop    %edi
 7ba:	5d                   	pop    %ebp
 ...
 7e9:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 7ec:	89 15 50 0d 00 00    	mov    %edx,0xd50
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 809:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 80c:	8b 3d 50 0d 00 00    	mov    0xd50,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 84f:	73 4f                	jae    8a0 <malloc+0xa0>
    ...
    if(p == freep)
 851:	8b 3d 50 0d 00 00    	mov    0xd50,%edi
 ...
 863:	e8 85 fc ff ff       	call   4ed <sbrk>
  if(p == (char*)-1)
 ...
 87a:	e8 f1 fe ff ff       	call   770 <free>
  return freep;
 87f:	8b 15 50 0d 00 00    	mov    0xd50,%edx
 ...
 8ac:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 8af:	89 15 50 0d 00 00    	mov    %edx,0xd50
}
 ...
 8bf:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 8c0:	c7 05 50 0d 00 00 54 	movl   $0xd54,0xd50
 8c7:	0d 00 00 
    base.s.size = 0;
 8ca:	bf 54 0d 00 00       	mov    $0xd54,%edi
    base.s.ptr = freep = prevp = &base;
 8cf:	c7 05 54 0d 00 00 54 	movl   $0xd54,0xd54
 8d6:	0d 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 8d9:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 8db:	c7 05 58 0d 00 00 00 	movl   $0x0,0xd58
 8e2:	00 00 00 
    if(p->s.size >= nunits){
